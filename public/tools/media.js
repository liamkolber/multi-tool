// Media Library — movies & TV from YTS, anime from AniList, in one search box.
//
// The largest of the tools: it brings its own sidebar (sub-nav + filters), its
...
const TEMPLATE = `
  <div class="app" id="app">
    <aside class="sidebar" id="sidebar">
      <div class="side-head">
        <div class="brand">
          ...
          <span class="brand-name">Media Library</span>
        </div>
        <button class="side-toggle" id="side-toggle" type="button"
          aria-label="Collapse sidebar" aria-expanded="true" title="Collapse sidebar">‹</button>
      </div>

      <nav class="source-toggle" id="source-toggle" role="tablist" aria-label="Library">
        <button class="src-btn active" data-src="all" type="button" title="All">
          <span class="src-icon">▦</span><span class="src-label">All</span>
        </button>
        <button class="src-btn" data-src="movies" type="button" title="Movies &amp; TV">
          <span class="src-icon">🎬</span><span class="src-label">Movies &amp; TV</span>
        </button>
        <button class="src-btn" data-src="anime" type="button" title="Anime">
          <span class="src-icon">🌸</span><span class="src-label">Anime</span>
        </button>
        <button class="src-btn" data-src="watchlist" type="button" title="Watchlist">
          <span class="src-icon">★</span><span class="src-label">Watchlist</span>
        </button>
      </nav>
      ...
      <div class="filters" id="filters">
        ...
        <label class="field" data-for="movies">
          <span>Quality</span>
          <select id="quality"></select>
        </label>
        <label class="field" data-for="movies">
          <span>Popular</span>
          <select id="movie-popular"></select>
        </label>
        <label class="field" data-for="movies">
          <span>Age rating</span>
          <select id="movie-rating"></select>
        </label>
        <label class="field" data-for="movies anime">
          <span>Genre</span>
          <select id="genre"></select>
        </label>
        <label class="field" data-for="anime">
          <span>Tag</span>
          <select id="anime-tag"></select>
        </label>
        <label class="field" data-for="anime">
          <span>Format</span>
        ...
        <label class="field" data-for="anime">
          <span>Status</span>
          <select id="anime-status"></select>
        </label>
        <label class="field" data-for="anime">
          <span>Rating</span>
          <select id="anime-rating"></select>
        </label>
        <label class="field" data-for="movies anime">
          <span>Sort by</span>
        ...
        <button class="pill" id="group-series" type="button" data-for="anime"
          title="Collapse same-series entries into one card">⧉ Group series</button>
        <button class="pill" id="streaming-only" type="button" data-for="anime"
          title="Only show titles with an official streaming source">▶ Streamable</button>

        <button id="reset" class="btn-ghost" type="button" title="Reset all filters">Reset</button>
      </div>
    </aside>
    ...
    <div class="main-col">
      <header class="topbar">
        <div class="search">
          <input id="search" type="search" placeholder="Search movies, TV &amp; anime…"
      ...
      <main class="content">
        <div class="results-bar">
          <span id="results-info" class="results-info">Loading…</span>
        </div>
...
const WATCHLIST_KEY = 'multitool:media-watchlist';
...
function cacheEls() {
  ...
  pageInfo: $('page-info'),
  modal: $('modal'),
  modalBody: $('modal-body'), // shell-owned, shared
});
}
...
function applySourceUI() {
  ...
  document.querySelectorAll('.filters [data-for]').forEach((el) => {
    const show = el.dataset.for.split(' ').includes(state.source);
  ...
  // All and Watchlist have no filters of their own, so the whole block goes
  // rather than leaving a bare heading and a Reset button behind.
...
const PREFS_KEY = 'multitool:media-prefs';
