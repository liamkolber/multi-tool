...
import { fetchComments, isYouTube, COMMENT_LIMITS } from '../comments.mjs';
...
async function handleComments(res, url) {
  ...
  const limit = COMMENT_LIMITS.includes(Number(url.searchParams.get('limit')))
  ...
  const out = await fetchComments(tools.ytdlp.path, target.url, { limit, sort });
