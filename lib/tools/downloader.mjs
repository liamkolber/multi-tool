...
import { open as openSession, close as closeSession, isOpen as sessionOpen, hasProfile, findBrowser, cookieSpec } from '../session.mjs';
...
export function classifyInstagram(raw) {
  ...
  if (first === 'p' || first === 'reel' || first === 'reels' || first === 'tv') {
    return second ? { kind: 'post', code: second } : { kind: 'unsupported' };
  }
  if (first === 'stories') {
...
async function handleSessionOpen(req, res) {
  ...
  const result = openSession(target);
