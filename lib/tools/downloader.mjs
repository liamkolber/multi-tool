...
import { fetchComments, isYouTube, COMMENT_LIMITS, isValidLimit } from '../comments.mjs';
...
// Exporting the session means spawning yt-dlp to decrypt the cookie store,
// which takes a second or two. A grid of six thumbnails asked for six of those
// at once and most of them lost the race — four of six previews came back
// blank. Held for a few minutes instead, and the in-flight export is shared so
// simultaneous callers wait on one process rather than starting their own.
const IG_SESSION_TTL = 5 * 60 * 1000;
let igSessionCache = { at: 0, cookies: null };
let igSessionInFlight = null;

async function igSession() {
  if (igSessionCache.cookies && Date.now() - igSessionCache.at < IG_SESSION_TTL) {
    return { ok: true, cookies: igSessionCache.cookies };
  }
  if (igSessionInFlight) return igSessionInFlight;

  igSessionInFlight = (async () => {
    ...
    igSessionCache = { at: Date.now(), cookies };
    return { ok: true, cookies };
  })();

  try {
    return await igSessionInFlight;
  } finally {
    igSessionInFlight = null;
  }
}

// A fresh sign-in invalidates whatever was cached.
export function forgetIgSession() {
  igSessionCache = { at: 0, cookies: null };
}
...
// Comments are fetched once and searched in the browser. Pulling them is
// seconds of work and the same request repeated for every keystroke would be
// absurd, so the answer is held long enough to search through.
const COMMENT_TTL = 10 * 60 * 1000;
const commentCache = new Map();

// How far a running fetch has got, so the browser can show a number climbing
// rather than a spinner that looks identical to a hang.
const commentProgress = new Map();

function handleCommentProgress(res, url) {
  const token = String(url.searchParams.get('token') || '');
  const p = commentProgress.get(token);
  return sendJson(res, 200, { status: 'ok', data: { seen: p ? p.seen : null, running: !!p } });
}

// A fetch that died without clearing its entry would otherwise sit there for
// the life of the process.
setInterval(() => {
  const cutoff = Date.now() - 10 * 60 * 1000;
  for (const [k, v] of commentProgress) if (v.at < cutoff) commentProgress.delete(k);
}, 60_000).unref();

async function handleComments(res, url) {
  ...
  if (!isYouTube(target.url)) {
    return sendJson(res, 400, { status: 'error', status_message: 'Comments are only read from YouTube links.' });
  }
  ...
  const limit = isValidLimit(asked) ? (String(asked) === 'all' ? 'all' : Number(asked)) : COMMENT_LIMITS[0];
  const sort = url.searchParams.get('sort') === 'new' ? 'new' : 'top';

  // A token lets the browser watch a long fetch instead of staring at a
  // spinner. Optional: without one this still works, just silently.
  const token = String(url.searchParams.get('token') || '').slice(0, 64);

  const key = `${target.url}|${limit}|${sort}`;
  const hit = commentCache.get(key);
  if (hit && Date.now() - hit.at < COMMENT_TTL) {
    return sendJson(res, 200, { status: 'ok', data: { ...hit.data, cached: true } });
  }

  if (token) commentProgress.set(token, { seen: 0, at: Date.now() });
  const out = await fetchComments(tools.ytdlp.path, target.url, {
    limit,
    sort,
    onProgress: (seen) => {
      if (token) commentProgress.set(token, { seen, at: Date.now() });
    },
  });
  if (token) commentProgress.delete(token);
  if (!out.ok) return sendJson(res, 502, { status: 'error', status_message: out.message });

  commentCache.set(key, { at: Date.now(), data: out.data });
  if (commentCache.size > 12) commentCache.delete(commentCache.keys().next().value);
...
async function handleDlApi(req, res, url) {
  ...
  if (route === 'comments') return handleComments(res, url);
  if (route === 'comments/progress') return handleCommentProgress(res, url);
