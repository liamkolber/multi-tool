...
async function handleSearch(res, url) {
  ...
  // first page from Danbooru and does its own paging inside it.
  ...
  }, `art:posts:${sort}:${fetchPage}:${limit}:${[...active].join('')}:${tag}:${extra}`);

  if (!out.ok) return sendJson(res, 502, { status: 'error', status_message: out.message });

  const raw = out.json;
  let posts = raw.filter((p) => isServable(p, active, config.blocked)).map(shapePost);

  // Whether the local sort saw everything there was, or only the first page.
  const sortExact = !localSort || raw.length < OVERFETCH;
  let hasNext;
