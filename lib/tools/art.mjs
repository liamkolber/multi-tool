...
// ══════════════════════════════════════════════════════════════════════════
//  CONFIGURATION — config/art-ratings.json, read live.
//
//  That file is the only place ratings are set. Editing it takes effect on the
//  next request: no restart, and nothing else to touch — the overlay toggles,
//  the Danbooru query, the response gate, the cache key and the labels all
//  follow from it.
//
...
const RATINGS_FILE = join(ROOT, 'config', 'art-ratings.json');

// Where a cold start lands if the file is missing or unreadable: the strictest
// option. A config problem must never fail open to something more permissive
// than was asked for.
const RATINGS_FALLBACK = ['g'];

// Re-read only when the file's mtime moves, so this costs one stat() per
// request rather than a parse.
let ratingsState = { mtime: -1, set: null, reported: '' };

function loadRatings() {
  let stamp;
  try {
    stamp = statSync(RATINGS_FILE).mtimeMs;
  } catch {
    // No file at all. Warn once, then hold whatever we already had.
    if (ratingsState.reported !== 'missing') {
      ratingsState.reported = 'missing';
      console.warn(`[art] ${RATINGS_FILE} not found — using ${RATINGS_FALLBACK.join(',')}`);
    }
    return ratingsState.set || new Set(RATINGS_FALLBACK);
  }

  if (stamp === ratingsState.mtime && ratingsState.set) return ratingsState.set;

  let list;
  try {
    const parsed = JSON.parse(readFileSync(RATINGS_FILE, 'utf8'));
    list = (Array.isArray(parsed.enabled) ? parsed.enabled : [])
    ...
    if (ratingsState.reported !== `parse:${stamp}`) {
      ratingsState.reported = `parse:${stamp}`;
      console.warn(`[art] config/art-ratings.json is not valid JSON (${err.message}) — keeping previous`);
    }
    ratingsState.mtime = stamp;
    return ratingsState.set || new Set(RATINGS_FALLBACK);
  }

  const bad = list.filter((r) => !RATINGS[r]);
  const good = list.filter((r) => RATINGS[r]);

  if (bad.length) {
    console.warn(`[art] ignoring unknown rating(s) ${bad.join(', ')} — valid: ${Object.keys(RATINGS).join(', ')}`);
  }
  if (!good.length) {
    console.warn(`[art] no valid ratings in config/art-ratings.json — using ${RATINGS_FALLBACK.join(',')}`);
  }

  const next = new Set(good.length ? good : RATINGS_FALLBACK);
  const changed = !ratingsState.set || [...next].join() !== [...ratingsState.set].join();
  if (changed) console.log(`[art] ratings now: ${[...next].join(', ')}`);

  ratingsState = { mtime: stamp, set: next, reported: '' };
  return next;
}
...
// Load-bearing as soon as the configured set goes past 'g'. At 'g' these never
// matched — rating:g + loli returns zero posts on Danbooru. At 's' and 'q'
// both return plenty, so this list is the only thing standing between a
// character-list button and sexualised art of characters who are minors.
//
// It catches what is tagged, which is not the same as catching the category:
// of the top 100 g/s/q posts for kanna_kamui, 23 carry `loli` and are dropped,
// while the rest stay — q-rated art of a child character that nobody tagged.
// Danbooru's tagging is inconsistent and this list cannot fix that.
...
async function handleSearch(res, url) {
  ...
  const out = await danbooru('/posts.json', {
    ...
    page: String(page),
  }, `art:posts:${sort}:${page}:${[...active].join('')}:${tag}`);

  if (!out.ok) return sendJson(res, 502, { status: 'error', status_message: out.message });

  const raw = out.json;
  const posts = raw.filter((p) => isServable(p, active)).map(shapePost);
