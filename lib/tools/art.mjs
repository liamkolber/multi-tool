...
// The one knob. Danbooru's four ratings, strictest first:
//
//   g  general       no nudity, nothing suggestive
//   s  sensitive     swimsuits, underwear, revealing outfits — not pornographic
//   q  questionable  partial nudity, strong suggestion, no explicit acts
//   e  explicit      pornographic
//
// Each is an exact bucket, not a threshold: asking for 's' alone would exclude
// 'g'. So this is a set, and widening means adding to it rather than swapping
// the value — ['g', 's'] serves general AND sensitive.
//
// Both the outgoing query and the incoming check read from here, so the two
// can no longer disagree. They used to: the query hardcoded rating:general
// while the check compared against this constant, which meant editing it alone
// produced an empty grid and no error.
const ALLOWED_RATINGS = new Set(['g']);

// Danbooru takes comma-separated rating letters as OR: rating:g,s is both.
const RATING_QUERY = `rating:${[...ALLOWED_RATINGS].join(',')}`;
...
function isServable(p) {
  if (!p || !ALLOWED_RATINGS.has(p.rating)) return false;
