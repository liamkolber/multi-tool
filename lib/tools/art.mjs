...
// Every image is relayed, both sources, because both file hosts refuse the
// browser and neither refuses this server.
//
// files.yande.re answers 307 to a cross-origin browser request. cdn.donmai.us
// answers 403 — to a plain fetch and to one carrying a browser User-Agent, but
// 200 to this app's own, which is Cloudflare bot management rejecting a UA
// that does not match the connection behind it.
//
// Two client-side fixes were shipped for this before the cause was understood
// and neither worked, because neither could be checked from here: a browser
// was the only thing that could tell. Relaying can be verified end to end, so
// that is what it does.
const viaProxy = (u) => (httpUrl(u) ? `/api/art/image?u=${encodeURIComponent(u)}` : null);
...
function shapeDanbooru(p) {
  ...
  return {
    ...
    preview: viaProxy(p.preview_file_url),
    large: viaProxy(p.large_file_url),
    full: ALLOWED_EXT.has(ext) ? viaProxy(p.file_url) : null,
    ext,
...
function shapeYandere(p) {
  ...
  return {
    ...
    preview: viaProxy(p.preview_url),
    large: viaProxy(p.sample_url || p.jpeg_url || p.file_url),
    full: ALLOWED_EXT.has(ext) ? viaProxy(p.file_url) : null,
    ext,
    bytes: p.file_size || null,
    width: p.width || null,
...
// Hosts whose bytes this server will relay. A strict allowlist, because an
// image proxy that will fetch any URL is an open proxy: it would let anything
// that can reach this endpoint pull from the machine's own network — including
// localhost services and anything else on the LAN — using this server as the
// caller.
...
// files.yande.re answers 307 to a cross-origin browser request, redirecting to
// the post page, so an <img> pointed straight at it renders nothing. The same
// URL fetched from here returns the file. referrerpolicy="no-referrer" was the
// obvious fix and did not work — the browser was still refused — so the bytes
// come through this server instead and the page only ever asks its own origin.
async function handleImage(req, res, url) {
  ...
  if (target.protocol !== 'https:' || !IMAGE_HOSTS.has(target.hostname)) {
    return sendJson(res, 403, { status: 'error', status_message: 'Not a permitted image host.' });
  }
  ...
  if (!upstream.ok || !type.startsWith('image/')) {
    // Whatever came back is not a picture; do not pass it on.
    return sendJson(res, 502, {
      status: 'error',
      status_message: `${target.hostname} returned ${upstream.status} ${type || 'no type'}`,
    });
  }
  ...
  res.writeHead(200, {
    'Content-Type': type,
    // These files are content-addressed by hash, so they never change.
    'Cache-Control': 'public, max-age=604800, immutable',
    ...(length ? { 'Content-Length': length } : {}),
  });

  // Streamed rather than buffered: originals run to several megabytes and there
  // is no reason to hold one in memory to hand it straight on.
...
async function handleApi(req, res, url) {
  ...
  if (route === 'image') return handleImage(req, res, url);
