...
// ══════════════════════════════════════════════════════════════════════════
//  CONFIGURATION — ratings are managed here and nowhere else.
//
//  Edit ENABLED below. That one line drives all of it: which toggles appear
//  in the overlay, what is asked of Danbooru, what the response gate accepts,
//  and the cache key. No other file needs touching — not the client, not the
//  CSS, not the tool's description.
// ══════════════════════════════════════════════════════════════════════════

// Danbooru's four ratings, strictest first. Each is an exact bucket rather
// than a threshold, which is why ENABLED is a list and not a level: turning on
...
// ─── THE KNOB ─────────────────────────────────────────────────────────────
// Drop a letter to remove that rating everywhere; add one to allow it.
const ENABLED = ['g', 's'];
// ──────────────────────────────────────────────────────────────────────────

// A typo in ENABLED would otherwise surface as an empty grid with no error,
// so it fails at startup instead — the whole point of a knob you change on a
// whim is that getting it wrong tells you immediately.
for (const r of ENABLED) {
  if (!RATINGS[r]) {
    throw new Error(`art.mjs: ENABLED lists an unknown rating "${r}" `
      + `(expected any of: ${Object.keys(RATINGS).join(', ')})`);
  }
}
if (!ENABLED.length) throw new Error('art.mjs: ENABLED must list at least one rating.');

const ALLOWED_RATINGS = new Set(ENABLED);
...
// never a union: ALLOWED_RATINGS is a ceiling and no query string can raise it,
...
function requestedRatings(raw) {
  const asked = String(raw || '')
    .split(',')
    .map((s) => s.trim().toLowerCase())
    .filter((r) => ALLOWED_RATINGS.has(r));
  return asked.length ? new Set(asked) : new Set(ALLOWED_RATINGS);
}

// Load-bearing as soon as ALLOWED_RATINGS goes past 'g'. At 'g' these never
...
async function handleSearch(res, url) {
  ...
  const active = requestedRatings(url.searchParams.get('ratings'));
