...
import { ROOT, sendJson, readBody, sameOrigin, resolveBinary } from '../core.mjs';

const PS_EXE = join(process.env.SystemRoot || 'C:\\Windows', 'System32', 'WindowsPowerShell', 'v1.0', 'powershell.exe');

// --- What kind of file is this? ---------------------------------------------
// Decided by extension first because it costs nothing and is right almost
// always; the probe below corrects it when the extension lies.
const IMAGE_EXT = new Set(['png', 'jpg', 'jpeg', 'webp', 'bmp', 'gif', 'tif', 'tiff', 'avif', 'heic']);
const AUDIO_EXT = new Set(['mp3', 'm4a', 'aac', 'opus', 'flac', 'wav', 'ogg', 'wma', 'aiff']);
const VIDEO_EXT = new Set(['mp4', 'mkv', 'webm', 'mov', 'avi', 'flv', 'wmv', 'm4v', 'mpg', 'mpeg', 'ts', 'm2ts']);
...
// --- Probing ----------------------------------------------------------------
// ffprobe gives clean JSON, but it isn't in bin/ unless get-tools.ps1 has been
// re-run since it started fetching it — and ffmpeg alone can still tell us
// everything we need, it just says it on stderr in prose. Both paths are
// supported so the tool works before anyone re-runs anything.

function runCapture(cmd, args, timeoutMs = 20_000) {
...
function parseFfmpegStderr(err) {
  ...
  const vcodec = vLine ? vLine[1] : null;
...
async function probeFile(path, tools) {
  let info = null;
  if (tools.ffprobe.found) {
    const { out } = await runCapture(tools.ffprobe.path, [
      '-v', 'quiet', '-print_format', 'json', '-show_format', '-show_streams', '--', path,
    ]);
    info = parseFfprobeJson(out);
  }
  if (!info && tools.ffmpeg.found) {
    const { err } = await runCapture(tools.ffmpeg.path, ['-hide_banner', '-i', path]);
...
const JOB_LIVE = new Set(['picking', 'starting', 'running']);
...
async function describe(path) {
  ...
  const probe = await probeFile(path, tools);
...
async function handleStart(req, res) {
  ...
  return runJob(job, opId, opts, described, tools, outPath);
}
