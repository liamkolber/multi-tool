...
import { writeCbz, naturalSort } from '../cbz.mjs';
...
const OPS = {
  ...
  // Pack a folder of pages into a comic archive. A cbz is a zip of images with
  // a different extension, so this is the one operation here that never touches
  // ffmpeg — the pages go in untouched, at their original quality.
  cbz: {
    kinds: ['folder'],
    label: 'Make CBZ',
    ext: () => 'cbz',
    // Named for the folder, since that is what a chapter is called.
    outName: (o, described) => described.name,
    async run(job, o, described, tools, outPath) {
      const wanted = new Set(
        (String(o.include || 'all') === 'webp' ? ['webp'] : CBZ_PAGE_EXT),
      );
      const pages = described.pages
        .filter((p) => wanted.has(p.ext))
        .sort((a, b) => naturalSort(a.name, b.name));

      if (!pages.length) throw new Error('No matching images in that folder.');

      // Readers order pages by filename, and a folder of "1.webp, 2.webp,
      // 10.webp" sorts correctly here but not in every reader — some sort as
      // plain text. Renumbering with a fixed width makes the order survive
      // whatever opens it.
      const pad = String(pages.length).length;
      const renumber = String(o.rename || 'keep') === 'number';

      return writeCbz(pages.map((p) => p.path), outPath, {
        cancelled: () => job.status === 'cancelled',
        nameFor: (i, path) => (renumber
          ? `${String(i + 1).padStart(pad, '0')}${extname(path)}`
          : basename(path)),
        onProgress: (done, total, name) => {
          job.pct = Math.round((done / total) * 100);
          job.page = `${done} / ${total}  ${name}`;
          broadcast(job);
        },
      });
    },
...
// What a comic archive may contain. Deliberately narrow: a stray .txt or
// .nfo in a download folder is not a page and readers choke on some of them.
const CBZ_PAGE_EXT = ['webp', 'jpg', 'jpeg', 'png', 'gif', 'avif', 'bmp'];
...
// An op that brings its own implementation instead of ffmpeg arguments.
// Everything about finishing a job is the same either way, so the two share
// nothing but this shape: set running, do the work, land in done, cancelled or
// error, and release the queue slot on every one of those paths.
async function runOwn(job, op, opts, described, tools, outPath) {
  job.output = outPath;
  ...
  try {
    const result = await op.run(job, opts, described, tools, outPath);
    if (job.status === 'cancelled') throw new Error('cancelled');
    job.status = 'done';
    job.pct = 100;
    job.outSize = (result && result.bytes) || (await stat(outPath)).size;
  } catch (err) {
    const stopped = job.status === 'cancelled' || err.message === 'cancelled';
    job.status = stopped ? 'cancelled' : 'error';
    if (!stopped) job.error = err.message;
...
function runJob(job, opId, opts, probe, tools, outPath) {
  ...
  const op = OPS[opId];
  if (op.run) return runOwn(job, op, opts, probe, tools, outPath);
...
// A folder is described by what is in it rather than by probing it: the pages
// are the interesting part, and running ffprobe over a few hundred of them to
// learn they are images would take far longer than packing them.
async function describeFolder(path) {
  ...
  for (const name of names) {
    ...
    if (!CBZ_PAGE_EXT.includes(ext)) continue;
    const full = join(path, name);
    try {
      const st = await stat(full);
      if (!st.isFile()) continue;
      pages.push({ name, path: full, ext, size: st.size });
      bytes += st.size;
    } catch { /* vanished between listing and stat */ }
  }

  if (!pages.length) return { error: 'No images in that folder.' };

  const byExt = {};
  for (const p of pages) byExt[p.ext] = (byExt[p.ext] || 0) + 1;
...
async function describe(path) {
  ...
  try {
    ...
    if (st.isDirectory()) return describeFolder(path);
...
async function handleProbe(req, res, url) {
  ...
  // describe() reports a folder by listing it, so only the file path needs an
  // encoder present.
  const tools = await getTools();
  const isDir = await stat(path).then((s) => s.isDirectory()).catch(() => false);
  if (!isDir && !tools.ffmpeg.found) {
...
async function handleStart(req, res) {
  ...
  // Packing a folder into an archive needs no encoder, so a missing ffmpeg
  // should not stand in its way.
  if (!op.run && !tools.ffmpeg.found) {
    return sendJson(res, 503, { status: 'error', status_message: 'ffmpeg is not installed.', missing: 'ffmpeg' });
  }
