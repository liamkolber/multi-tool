...
// A storyboard is the same idea at higher density: one strip of frames the
// scrubber can index into, so dragging the timeline shows where you are landing.
const BOARD_COLS = 10;
const BOARD_TILE_W = 160;
const BOARD_MIN = 20;
const BOARD_MAX = 160;
// One frame per five seconds, up to the cap. Most files get exactly that; a
// feature-length one degrades to a coarser spacing rather than spending a
// minute building granularity nobody scrubs at.
const BOARD_SECONDS_PER_FRAME = 5;

// Windows refuses a command line over 32,767 characters, and it refuses it at
// CreateProcess: the spawn fails instantly with no exit code and no stderr, so
// it reads as "ffmpeg could not do it" rather than as a limit being hit.
//
// Each tile costs one "-ss T -i <path>" plus its filter-graph entry, so the
// ceiling depends on how deeply the file happens to be nested — measured at
// roughly (path length + 90) characters per tile. Budgeting below the hard cap
// keeps a file buried in a long folder tree from silently getting no
// storyboard at all.
const CMDLINE_BUDGET = 30_000;
const CMDLINE_PER_TILE_OVERHEAD = 90;
...
// --- Storyboard ---------------------------------------------------------------
// Everything about the grid follows from the duration, so the client can be
// told the shape without the image having been built yet.
// How many tiles this particular path can afford before the command line is
// longer than Windows will accept.
function boardCeiling(path) {
  const perTile = String(path).length + CMDLINE_PER_TILE_OVERHEAD;
  return Math.max(BOARD_MIN, Math.floor(CMDLINE_BUDGET / perTile));
}

function boardPlan(f) {
  const duration = f.duration || 0;
  const wanted = Math.round(duration / BOARD_SECONDS_PER_FRAME) || BOARD_MIN;
  const count = Math.max(
    BOARD_MIN,
    Math.min(BOARD_MAX, boardCeiling(f.path), wanted),
  );
  const cols = Math.min(BOARD_COLS, count);
  const rows = Math.ceil(count / cols);

  // scale=W:-2 keeps the aspect and rounds the height to an even number, which
  // the encoder requires — so the client is told the same rounding.
  const aspect = f.width && f.height ? f.height / f.width : 9 / 16;
  const tileH = Math.max(2, Math.round((BOARD_TILE_W * aspect) / 2) * 2);

  return { count, cols, rows, tileW: BOARD_TILE_W, tileH, interval: duration / count, duration };
}

function boardArgs(f, plan, out) {
  const args = ['-hide_banner', '-loglevel', 'error', '-y'];
  for (let i = 0; i < plan.count; i++) {
    args.push('-ss', (((i + 0.5) / plan.count) * plan.duration).toFixed(3), '-i', f.path);
  }

  const parts = [];
  for (let i = 0; i < plan.count; i++) {
    parts.push(`[${i}:v]scale=${plan.tileW}:-2,trim=end_frame=1,setpts=PTS-STARTPTS[t${i}]`);
  }
  const chain = Array.from({ length: plan.count }, (_, i) => `[t${i}]`).join('');
  parts.push(`${chain}concat=n=${plan.count}:v=1:a=0,tile=${plan.cols}x${plan.rows}[board]`);

  args.push('-filter_complex', parts.join(';'), '-map', '[board]', '-frames:v', '1', '-q:v', '6', '--', out);
  return args;
}

function runBoard(f, plan, out, tools) {
  return new Promise((resolve) => {
    let child;
    try {
      child = spawn(tools.ffmpeg.path, boardArgs(f, plan, out), { windowsHide: true });
    } catch {
      return resolve(false);
    }
    // A hundred seeks into a long file is real work, but each one is cheap.
...
async function handleBoard(req, res, url) {
  const f = indexedFile(url.searchParams.get('path'));
  if (!f || f.kind !== 'video' || f.unreadable || !f.duration) {
    return sendJson(res, 404, { status: 'error', status_message: 'No storyboard for that.' });
  }

  const plan = boardPlan(f);
  const out = join(THUMB_DIR, `${thumbKey(f)}-board.jpg`);
  const wantsImage = url.searchParams.get('img') === '1';
  ...
  if (!built) {
    const tools = await getMediaTools();
    if (!tools.ffmpeg.found) {
      return sendJson(res, 503, { status: 'error', status_message: 'ffmpeg is not installed.' });
    }
    await thumbSlot();
    try {
      await mkdir(THUMB_DIR, { recursive: true });
      built = await runBoard(f, plan, out, tools);
    } finally {
      releaseThumbSlot();
    }
  }
  ...
  if (!built) {
    return sendJson(res, 404, { status: 'error', status_message: 'Could not build a storyboard.' });
  }
  if (wantsImage) return sendImage();

  return sendJson(res, 200, {
    status: 'ok',
    data: {
      ...plan,
      src: `/api/library/board?img=1&path=${encodeURIComponent(f.path)}`,
    },
  });
}
...
async function handleApi(req, res, url) {
  ...
  if (route === 'board') return handleBoard(req, res, url);
