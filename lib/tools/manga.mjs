...
async function mangadexFor(anilistId, titles) {
  ...
  try {
    ...
    // The entry's own page, but only when there is something in English to
    // find there. availableTranslatedLanguages is the check: it lists the
    // languages an entry actually has chapters in, and plenty have none or
    // only Japanese — a link to a page you cannot read is worse than no link.
    //
    // This counts fan translations, which is why it gates a link and is not
    // reported as availability. Whether a title can be read legitimately is
    // what the licensed links above answer; this only decides whether the
    // cross-reference is worth offering.
    const languages = attrs.availableTranslatedLanguages || [];
    const data = {
      links,
      url: languages.includes('en') ? `https://mangadex.org/title/${match.id}` : null,
    };
...
async function handleDetails(res, url) {
  ...
  if (extra) {
    ...
    data.mangadexUrl = extra.url || null;
  }
