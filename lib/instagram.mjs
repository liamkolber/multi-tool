...
async function apiGet(url, cookies) {
  ...
  try {
    ...
    if (res.status === 429) {
      // Not a failure to fix, a pace to keep. Instagram throttles the profile
      // lookup especially hard, and retrying is what turns a short cool-off
      // into a long one.
      ...
      return {
        ...
        rateLimited: true,
        message: wait
          ? `Instagram is rate-limiting this. Try again in about ${Math.ceil(wait / 60)} minute(s).`
          : 'Instagram is rate-limiting this. Wait a few minutes before trying again — '
            + 'retrying now only extends it.',
      };
    }
...
export async function fetchPost(shortcode, cookies) {
  ...
  // A post already describes its owner: numeric id, whether the account is
  // private, and whether you follow it. That is everything web_profile_info
  // was being asked for, arriving from an endpoint that is not refused — so
  // it is kept, and stories for this account no longer need the blocked one.
  const owner = media.user || {};
  if (owner.username && (owner.pk || owner.pk_id)) {
    const fs = owner.friendship_status || {};
    rememberUserId(String(owner.username).toLowerCase(), {
      id: String(owner.pk || owner.pk_id),
...
// An account's numeric id is permanent, and web_profile_info — the only route
// found that reliably yields it — is the endpoint Instagram throttles first.
//
// Two alternatives were tried and neither works: feed/user/<name>/username/
// and feed/reels_tray/ both answer 200 with the web app's HTML rather than
// JSON, which is a 200 that means nothing. Scraping the id out of the profile
// page looked promising until the number it produced turned out not to be the
// account's id at all, just the first long "id" field on the page.
//
// So the limit cannot be routed around, only paid once. Ids are kept on disk:
// resolve an account while the limit is clear and every later story fetch for
// it skips the throttled endpoint entirely, across restarts.
const userIdCache = new Map();
const USER_ID_FILE = join(tmpdir(), 'multitool-ig-users.json');

try {
  for (const [k, v] of Object.entries(JSON.parse(readFileSync(USER_ID_FILE, 'utf8')))) {
    userIdCache.set(k, v);
  }
} catch { /* first run, or the file went away — neither is a problem */ }

function rememberUserId(key, data) {
  userIdCache.set(key, data);
  try {
    writeFileSync(USER_ID_FILE, JSON.stringify(Object.fromEntries(userIdCache)));
...
export async function fetchUserId(username, cookies) {
  ...
  const known = userIdCache.get(key);
  ...
  // This endpoint is refused outright for some accounts — measured: 429 to
  // every header combination the web client sends, at any hour, while
  // media/info answers 200 with the same session. Waiting does not help, so
  // say the thing that does: fetch any post by this account and the id comes
  // from there instead, permanently.
  if (!out.ok) {
    return out.rateLimited
      ? {
        ok: false,
        message: `Instagram will not look up @${username} directly. Paste a link to any `
          + 'post by them first — that gives this the account id it needs, and stories '
