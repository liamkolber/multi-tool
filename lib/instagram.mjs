...
export async function fetchMedia(url, cookies) {
  const res = await fetch(url, {
    headers: { Cookie: cookies, 'User-Agent': UA, Referer: 'https://www.instagram.com/' },
  });
  if (!res.ok) throw new Error(`Instagram's CDN replied ${res.status}.`);
  return res;
}
